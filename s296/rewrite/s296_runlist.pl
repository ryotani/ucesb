...
chomp ($FILENAME);
if ($FILENAME !~ /(s296_.*).gst/) {	# filename has to match certain pattern
	die ("Error: The list name you specified does not match 's296_*\.gst'\n");
}
my $LISTNAME=$1;
...
for ($i=0; $i<=$#lines; $i++) {
	my $LINE = $lines[$i];
	if ($LINE =~ /(\d+):\s+(.*\/(\w+)\/(((run|r_*)(\d*))_(\d*)\.lmd)).*/) { #r_*
#	              1        2    3      456        7      8
		if ($7 ne $OLDRUN) {
			#print "$5\n";
			...
			printf(testFILESout "$7");	#print run number at the beginning of each line
			$NUMRUNS++;
		}
		if (isBlackListed($4)) {
			next;
