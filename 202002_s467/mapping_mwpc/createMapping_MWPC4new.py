...
if(lenTabCh==64):
    for pad in range (0,64):
        ...
        outMWPC4X.write('SIGNAL(SOFMWPC4_Plane1_Q'+str(288-pad)+',sofia_mwpc.data.vmmr8.data['+str(index)+'],DATA12);\n')

    for pad in range (0,64):
        #MWPC_X2
        index = TabCh[pad]+64*MWPC4_X4_bus
        outMWPC4X.write('SIGNAL(SOFMWPC4_Plane1_Q'+str(224-pad)+',sofia_mwpc.data.vmmr8.data['+str(index)+'],DATA12);\n')
    
    for pad in range (0,64):
        #MWPC_X3
        index = TabCh[pad]+64*MWPC4_X3_bus
        outMWPC4X.write('SIGNAL(SOFMWPC4_Plane1_Q'+str(160-pad)+',sofia_mwpc.data.vmmr8.data['+str(index)+'],DATA12);\n')

    for pad in range (0,64):
        #MWPC_X4
        index = TabCh[pad]+64*MWPC4_X2_bus
        outMWPC4X.write('SIGNAL(SOFMWPC4_Plane1_Q'+str(96-pad)+',sofia_mwpc.data.vmmr8.data['+str(index)+'],DATA12);\n')
