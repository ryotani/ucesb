...
FEBEX3_CALIFA_BASE(){
	...
	MEMBER(DATA16 tot[NUM_SFP_MODULES*FEBEX_MODULES_PER_SFP*CHANNELS_PER_FEBEX]   ZERO_SUPPRESS);
	...
	if(DATASIZE.sizebytes>0){
		if(HEADER.submemory_id==0xff){
			...
			if(HEADER_MAGIC.magic==0x115A){
				...
				ENCODE(tot[HEADER.sfp_id*FEBEX_MODULES_PER_SFP*CHANNELS_PER_FEBEX+HEADER.module_id*CHANNELS_PER_FEBEX+HEADER.submemory_id], (value=HEADER_TOT.tot));
				}
