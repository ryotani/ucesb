...
void user_pedestals(int det_number, int raw[NUM_CHANNELS],int sam,int gtb,int sidnum)
{
  ...
  count[det_number]++;
  //printf("detector = %dcount = %d / %d\n",det_number, count[det_number],NUM_PEDESTAL_DATA);
	  ...
	  if(done[det_number]==0)
	    {
	      ...
	      fprintf (human,"detector = %d, gtb = %d, siderem = %d, strip = %d: %6.1f+/-%4.1f , +/-%4.1f (%04x)\n",
		       det_number,gtb,sidnum,i,
