...
# see the s318_runlist.pl helping script.
#
#
#
# AUTHOR: Felix Wamers
...
#TARGETDIR="/d/land2/s318/lmd"
#TARGETDIR="/lustre/land/s318/lmd"
...
echo "$LISTHANDLE"
PERLDUMMY="`./s318_runlist.pl $FILEHANDLE $BLACKHANDLE`" #create file list in a different format for easier processing
...
do
	...
	MRGRWRT=""$UCESBBASE"unpackexps/s318/s318 --merge=6 $SOURCE --allow-errors --output=excl=cros3,eventcut="$EVENTCUT","$TARGETDIR"/s318_mrgrwt_"$thisRUN"_"$firstFILE".lmd"	#File numbering for each merged run will start at the number of the first original file belonging to that run
