...
{
  // UINT32 sec;
  READ_FROM_BUFFER(735,uint32 ,sec);
  // UINT32 frac;
  READ_FROM_BUFFER(736,uint32 ,frac);
}
...
void NTP_MESSAGE::__unpack(__data_src_t &__buffer,uint32 id)
{
  ...
  // {
    ...
    // 24_31: id = MATCH(id);
  // }
  READ_FROM_BUFFER_FULL(747,uint32 ,header,header.u32);
  CHECK_BITS_EQUAL(746,header.id,id);
  CHECK_UNNAMED_BITS_ZERO(747,header.u32,0x00fe0000);
  // MARK_COUNT(start);
  void *__mark_start = __buffer._data;
  // UINT32 server_id;
  READ_FROM_BUFFER(751,uint32 ,server_id);
  // UINT32 sspp
  // {
    //  0_07: precision;
    //  8_15: poll;
    ...
    // 30_31: leap;
  // }
  READ_FROM_BUFFER_FULL(763,uint32 ,sspp,sspp.u32);
  CHECK_BITS_EQUAL(761,sspp.version,3);
  CHECK_UNNAMED_BITS_ZERO(763,sspp.u32,0x00f00000);
  // UINT32 root_dly
  // {
    //  0_15: frac;
    // 16_31: sec;
  // }
  READ_FROM_BUFFER_FULL(769,uint32 ,root_dly,root_dly.u32);
  // UINT32 root_disp
  // {
    //  0_15: frac;
    // 16_31: sec;
  // }
  READ_FROM_BUFFER_FULL(774,uint32 ,root_disp,root_disp.u32);
  // UINT32 refid;
  READ_FROM_BUFFER(775,uint32 ,refid);
  // reftime = NTP64_TIME();
  UNPACK_DECL(777,NTP64_TIME,reftime);
  // origtime = NTP64_TIME();
  UNPACK_DECL(778,NTP64_TIME,origtime);
  // rectime = NTP64_TIME();
  UNPACK_DECL(779,NTP64_TIME,rectime);
  // tmittime = NTP64_TIME();
  UNPACK_DECL(780,NTP64_TIME,tmittime);
  // rxtime = NTP64_TIME();
  UNPACK_DECL(783,NTP64_TIME,rxtime);
  // if(header.quick_burst)

  if (header.quick_burst)
  {
    // orig2time = NTP64_TIME();
    UNPACK_DECL(787,NTP64_TIME,orig2time);
    // rec2time = NTP64_TIME();
    UNPACK_DECL(788,NTP64_TIME,rec2time);
    // tmit2time = NTP64_TIME();
    UNPACK_DECL(789,NTP64_TIME,tmit2time);
    // rx2time = NTP64_TIME();
    UNPACK_DECL(790,NTP64_TIME,rx2time);
  }
  // MARK_COUNT(end);
  void *__mark_end = __buffer._data;
  // CHECK_COUNT(header.word_count,start,end,0,4)
  CHECK_WORD_COUNT(795,header.word_count,start,end,0,4);
}
...
{
  // UINT32 soft_latches;
  READ_FROM_BUFFER(803,uint32 ,soft_latches);
  // UINT32 hard_latches;
  READ_FROM_BUFFER(804,uint32 ,hard_latches);
  // UINT32 clock_ticks;
  READ_FROM_BUFFER(805,uint32 ,clock_ticks);
  // before = NTP64_TIME();
  UNPACK_DECL(806,NTP64_TIME,before);
  // after = NTP64_TIME();
  UNPACK_DECL(807,NTP64_TIME,after);
}
...
void RANDOM_TCAL::__unpack(__data_src_t &__buffer,uint32 id)
{
  ...
  // {
    ...
    // 24_31: id = MATCH(id);
  // }
  READ_FROM_BUFFER_FULL(824,uint32 ,header,header.u32);
  CHECK_BITS_EQUAL(823,header.id,id);
  CHECK_UNNAMED_BITS_ZERO(824,header.u32,0x00f00000);
  // MARK_COUNT(start);
  void *__mark_start = __buffer._data;
  ...
  if (header.has_tstamp1)
  {
    ...
    UNPACK_DECL(830,RANDOM_TCAL_TSTAMP_CLOCK,tstamp1);
  }
  ...
  if (header.has_tstamp2)
  {
    ...
    UNPACK_DECL(834,RANDOM_TCAL_TSTAMP_CLOCK,tstamp2);
  }
  // MARK_COUNT(end);
  void *__mark_end = __buffer._data;
  // CHECK_COUNT(header.word_count,start,end,0,4)
  CHECK_WORD_COUNT(839,header.word_count,start,end,0,4);
}
...
void SOFT_SCALER32::__unpack(__data_src_t &__buffer,uint32 channels)
{
  ...
  for (uint32 index = 0; index < (uint32) (channels); ++index)
  {
    ...
    READ_FROM_BUFFER_FULL(868,uint32 ,ch_data,ch_data.u32);
    {
      typedef __typeof__(*(&(data))) __array_t;
      typedef typename __array_t::item_t __item_t;
      __item_t &__item = data.insert_index(867,index);
      __item.value = ch_data.value;
    }
  }
}
...
{
  // UINT16 tpat;
  READ_FROM_BUFFER(708,uint16 ,tpat);
  // UINT16 tprev;
  READ_FROM_BUFFER(709,uint16 ,tprev);
  // UINT16 tnext;
  READ_FROM_BUFFER(710,uint16 ,tnext);
  // UINT16 tprev2;
  READ_FROM_BUFFER(711,uint16 ,tprev2);
  // if(EXTERNAL has_data)

  if (has_data())
  {
    // tPHI7079 = CAMAC_PHILLIPS_7164(channels=16,mark_channel_no=1);
    UNPACK_DECL(716,CAMAC_PHILLIPS_7164,tPHI7079,/*channels*/16,/*mark_channel_no*/1);
    // aPHI12160 = CAMAC_PHILLIPS_7164(channels=14,mark_channel_no=1);
    UNPACK_DECL(717,CAMAC_PHILLIPS_7164,aPHI12160,/*channels*/14,/*mark_channel_no*/1);
    // qSIA0415 = CAMAC_SILENA_4418(channels=8,mark_channel_no=0);
    UNPACK_DECL(720,CAMAC_SILENA_4418,qSIA0415,/*channels*/8,/*mark_channel_no*/0);
    // qSIA0418 = CAMAC_SILENA_4418(channels=8,mark_channel_no=0);
    UNPACK_DECL(721,CAMAC_SILENA_4418,qSIA0418,/*channels*/8,/*mark_channel_no*/0);
    // qSIA0458 = CAMAC_SILENA_4418(channels=8,mark_channel_no=0);
    UNPACK_DECL(722,CAMAC_SILENA_4418,qSIA0458,/*channels*/8,/*mark_channel_no*/0);
    // tSIA0400 = CAMAC_SILENA_4418(channels=8,mark_channel_no=0);
    UNPACK_DECL(723,CAMAC_SILENA_4418,tSIA0400,/*channels*/8,/*mark_channel_no*/0);
    // tSIA0426 = CAMAC_SILENA_4418(channels=8,mark_channel_no=0);
    UNPACK_DECL(724,CAMAC_SILENA_4418,tSIA0426,/*channels*/8,/*mark_channel_no*/0);
    // tSIA0401 = CAMAC_SILENA_4418(channels=8,mark_channel_no=0);
    UNPACK_DECL(725,CAMAC_SILENA_4418,tSIA0401,/*channels*/8,/*mark_channel_no*/0);
  }
}
...
void LAND_CAMAC_SCALER::__unpack(__data_src_t &__buffer)
{
  ...
  UNPACK_DECL(874,CAMAC_LECROY_4434,scaler0,/*channels*/32);
  // scaler1 = CAMAC_LECROY_4434(channels=32);
  UNPACK_DECL(875,CAMAC_LECROY_4434,scaler1,/*channels*/32);
  // scaler2 = CAMAC_LECROY_4434(channels=32);
  UNPACK_DECL(876,CAMAC_LECROY_4434,scaler2,/*channels*/32);
  // if(EXTERNAL has_timestamp)

  if (has_timestamp())
  {
    // UINT32 timestamp;
    READ_FROM_BUFFER(885,uint32 ,timestamp);
    // UINT32 endianess
    // {
      //  0_31: 0x87654321;
    // }
    READ_FROM_BUFFER_FULL(886,uint32 ,endianess,endianess.u32);
    CHECK_BITS_EQUAL(886,endianess.unnamed_0_31,0x87654321);
  }
}
...
{
  // UINT32 timestamp;
  READ_FROM_BUFFER(892,uint32 ,timestamp);
  // UINT32 endianess
  // {
    //  0_31: 0x87654321;
  // }
  READ_FROM_BUFFER_FULL(893,uint32 ,endianess,endianess.u32);
  CHECK_BITS_EQUAL(893,endianess.unnamed_0_31,0x87654321);
}
...
{
  // UINT32 info;
  READ_FROM_BUFFER(845,uint32 ,info);
  // select several
  ...
  for ( ; ; )
  {
    ...
    {
      case 1:
        UNPACK_DECL(849,RANDOM_TCAL,rtcal,/*id*/231);
        break;
      case 2:
        UNPACK_DECL(850,NTP_MESSAGE,ntp,/*id*/239);
        break;
    }
  }
}
...
void LAND_FASTBUS1::__unpack(__data_src_t &__buffer)
{
  ...
  READ_FROM_BUFFER_FULL(936,uint32 ,zero,zero.u32);
  CHECK_BITS_EQUAL(936,zero.unnamed_0_31,0);
  // select several
  ...
  for ( ; ; )
  {
    ...
    {
    uint32 __match_peek;
    PEEK_FROM_BUFFER(964,uint32,__match_peek);
    ...
    {
      case 1:
        UNPACK_DECL(942,FASTBUS_LECROY_1875,T87044,/*geom*/4,/*channels*/64);
        break;
      case 2:
        UNPACK_DECL(943,FASTBUS_LECROY_1875,T87024,/*geom*/5,/*channels*/64);
        break;
      case 3:
        UNPACK_DECL(944,FASTBUS_LECROY_1875,T81800,/*geom*/6,/*channels*/64);
        break;
      case 4:
        UNPACK_DECL(945,FASTBUS_LECROY_1885,Q47342,/*geom*/7,/*channels*/96);
        break;
      case 5:
        UNPACK_DECL(946,FASTBUS_LECROY_1885,Q11111,/*geom*/8,/*channels*/96);
        break;
      case 6:
        UNPACK_DECL(947,FASTBUS_LECROY_1885,Q47264,/*geom*/9,/*channels*/96);
        break;
      case 7:
        UNPACK_DECL(948,FASTBUS_LECROY_1885,Q46848,/*geom*/10,/*channels*/96);
        break;
      case 8:
        UNPACK_DECL(949,FASTBUS_LECROY_1875,T81855,/*geom*/12,/*channels*/64);
        break;
      case 9:
        UNPACK_DECL(950,FASTBUS_LECROY_1885,Q17320,/*geom*/13,/*channels*/96);
        break;
      case 10:
        UNPACK_DECL(951,FASTBUS_LECROY_1885,Q47315,/*geom*/14,/*channels*/96);
        break;
      case 11:
        UNPACK_DECL(952,FASTBUS_LECROY_1885,Q15372,/*geom*/15,/*channels*/96);
        break;
      case 12:
        UNPACK_DECL(953,FASTBUS_LECROY_1885,Q46962,/*geom*/16,/*channels*/96);
        break;
      case 13:
        UNPACK_DECL(954,FASTBUS_LECROY_1885,Q15358,/*geom*/17,/*channels*/96);
        break;
      case 14:
        UNPACK_DECL(955,FASTBUS_LECROY_1885,Q46993,/*geom*/18,/*channels*/96);
        break;
      case 15:
        UNPACK_DECL(956,FASTBUS_LECROY_1875,T87047,/*geom*/19,/*channels*/64);
        break;
      case 16:
        UNPACK_DECL(957,FASTBUS_LECROY_1875,T48854,/*geom*/20,/*channels*/64);
        break;
      case 17:
        UNPACK_DECL(958,FASTBUS_LECROY_1875,T81808,/*geom*/21,/*channels*/64);
        break;
      case 18:
        UNPACK_DECL(959,FASTBUS_LECROY_1875,T81859,/*geom*/22,/*channels*/64);
        break;
      case 19:
        UNPACK_DECL(960,FASTBUS_LECROY_1875,T81795,/*geom*/23,/*channels*/64);
        break;
      case 20:
        UNPACK_DECL(961,FASTBUS_LECROY_1875,T81806,/*geom*/24,/*channels*/64);
        break;
      case 21:
        UNPACK_DECL(962,FASTBUS_LECROY_1875,T48834,/*geom*/25,/*channels*/64);
        break;
    }
  }
}
...
void LAND_FASTBUS_ERROR::__unpack(__data_src_t &__buffer)
{
  ...
  // {
    ...
    //    16: init_error;
  // }
  READ_FROM_BUFFER_FULL(908,uint32 ,error_code,error_code.u32);
  CHECK_UNNAMED_BITS_ZERO(908,error_code.u32,0xfffefff0);
  // UINT32 error_num;
  READ_FROM_BUFFER(909,uint32 ,error_num);
}
...
void LAND_FASTBUS_ERROR_NGF::__unpack(__data_src_t &__buffer)
{
  ...
  // {
    //  0_31: 0x10000000;
  // }
  READ_FROM_BUFFER_FULL(917,uint32 ,magic,magic.u32);
  CHECK_BITS_EQUAL(916,magic.unnamed_0_31,0x10000000);
  // UINT32 error_code
  // {
    //     0: error;
    //     1: uninitialized;
    ...
    //     9: word_lost;
  // }
  READ_FROM_BUFFER_FULL(930,uint32 ,error_code,error_code.u32);
  CHECK_UNNAMED_BITS_ZERO(930,error_code.u32,0xfffffc00);
}
  ...
  // fastbus1 = LAND_FASTBUS1(type=32,subtype=0xc1c);
{
  ...
  switch (__match_no)
  {
    case 1:
      ...
      UNPACK_SUBEVENT_DECL(977,0,LAND_CAMAC_CONVERTER,camac);
      break;
    case 2:
      UNPACK_SUBEVENT_CHECK_NO_REVISIT(984,LAND_CAMAC_SCALER,camac_scalers,1);
      UNPACK_SUBEVENT_DECL(984,0,LAND_CAMAC_SCALER,camac_scalers);
      break;
    case 3:
      UNPACK_SUBEVENT_CHECK_NO_REVISIT(987,LAND_FASTBUS_ERROR,fastbus_error,2);
      UNPACK_SUBEVENT_DECL(987,0,LAND_FASTBUS_ERROR,fastbus_error);
      break;
    case 4:
      UNPACK_SUBEVENT_CHECK_NO_REVISIT(989,LAND_FASTBUS1,fastbus1,3);
      UNPACK_SUBEVENT_DECL(989,0,LAND_FASTBUS1,fastbus1);
      break;
  }
  return 0;
}
