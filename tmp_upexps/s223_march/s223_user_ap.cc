...
void C_F_ERROR(INTS4 errnum,INTU4 mask,CHARS *str)
{
  fprintf(stderr,"AUTO: %s\n",str);
}

void C_F_ERROR_FMT(INTS4 errnum,INTU4 mask,CHARS *str,...)
{
  va_list ap;
...
auto_pedestal_adjust_channel apac[5][1024];
...
void user_pedestals(int det_number, int raw[NUM_CHANNELS])
{
  ...
  for (int i = 0; i < NUM_CHANNELS; i++)
    apac[det_number][i].collect_data(raw[i]);
