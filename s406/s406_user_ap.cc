...
#define NUM_SST_DETECTORS 8
#define NUM_BRANCH_1      8
...
void user_pedestals(int det_number, int raw[NUM_CHANNELS],int branch,int sam,int gtb,int sidnum)
{
  ...
  if ((count[det_number] & (NUM_PEDESTAL_DATA-1)) == 0)
    {
      FILE* ped;
      ped = fopen(det_number <= NUM_BRANCH_1 ? "sidped.txt" : "sidped_2.txt","a");
      FILE* sig;
      sig = fopen(det_number <= NUM_BRANCH_1 ? "sidsig.txt" : "sidsig_2.txt","a");
      FILE* sig_r;
      sig_r = fopen(det_number <= NUM_BRANCH_1 ? "sidsig_r.txt" : "sidsig_r_2.txt","a");
      FILE* human;
	  ...
	  if(done[det_number]==0)
	    {
	      ...
	      fprintf (human,"detector = %d, branch = %d, gtb = %d, siderem = %d, strip = %d: %6.1f+/-%4.1f , +/-%4.1f (%04x)\n",
		       det_number,det_number <= NUM_BRANCH_1,gtb,sidnum,i,
